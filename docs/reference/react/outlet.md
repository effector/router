...
```tsx
import { Outlet, Link } from '@effector/router-react';
...
When no child route is active and no child declares a fallback, Outlet renders
nothing:
...
To decide it imperatively instead:
...
function ParentComponent() {
  ...
  return (
    <div>
      <h1>Parent</h1>
      {hasActiveChild ? <Outlet /> : <div>Select an item from the sidebar</div>}
    </div>
  );
}
```
