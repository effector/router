...
```ts
import { compile, ParseUrlParams } from '@effector/router-paths';
...
Convert paths from effector/router format to express or other path format.
...
// Convert effector/router patterns to Express format
...
### effector/router-Router to Express Conversion

When you need to use effector/router paths with Express.js or generate Express-compatible routes, use `convertPath`:

| effector/router-Router Pattern | Express Pattern | Description              |
| ------------------------------ | --------------- | ------------------------ |
...
// effector/router paths with advanced features
const routerPaths = [
  '/users/:id<number>',
  '/files/:path+',
...
const expressPaths = routerPaths.map((path) => convertPath(path, 'express'));
...
Convert an effector/router path pattern to another router format.

**Parameters:**

- `path: string` - The effector/router path pattern to convert
...
- [createRoute](/core/create-route) - Use with effector/router
